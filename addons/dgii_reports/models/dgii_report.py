...
class DgiiReport(models.Model):
        ...
        for rec in self:
            ...
            for inv in invoice_ids:
                ...
                values = {
                    ...
                    "modified_invoice_number": inv.l10n_do_origin_ncf if inv.move_type == "in_refund" else False,
                    ...
                    "purchase_perceived_itbis": inv.withholded_itbis,
    ...
    def _convert_to_user_currency(self, base_currency, amount, date):
        ...
        ctx = context.copy()
        value = base_currency_id.with_context(ctx)._convert(amount, user_currency_id, self.company_id, date)
        return value
    ...
    def include_payment(invoice_id, payment_id):
        ...
        p_date = payment_id.payment_date or payment_id.date
    ...
    def _get_sale_payments_forms(self, invoice_id):
        ...
        if invoice_id.move_type == 'out_invoice':
            try:
                payment_data = invoice_id.pos_order_ids.payment_ids
            except:
                payment_data = invoice_id._get_invoice_payment_widget()
                
            for payment in payment_data:
                if type(payment) is dict:
            ...
            try:                
                if abs(sum(invoice_id.pos_order_ids.payment_ids.mapped('amount')) - invoice_id.amount_residual) != 0:
                    payments_dict['credit'] += self._convert_to_user_currency(
    ...
    def _process_op_dict(self, args, invoice):
        ...
        if invoice.l10n_latam_document_type_id and invoice.move_type != "out_refund":
            ...
            op_dict[invoice.l10n_latam_document_type_id.l10n_do_ncf_type]["amount"] += invoice.amount_untaxed_signed
            ...
            for inv in invoice_ids:
                if (
                    ...
                    or (
                        ...
                        and inv.l10n_latam_document_type_id.l10n_do_ncf_type in ['minor', 'normal']
                    )
                ):
                    inv.fiscal_status = "blocked" if not inv.fiscal_status else inv.fiscal_status
                    line += 1
    ...
    def _compute_609_data(self):
        for rec in self:
            ...
            invoice_ids = self._get_invoices('posted', ["in_invoice", "in_refund"], False).filtered(
                lambda inv: (inv.partner_id.country_id.code != "DO")
                and (inv.invoice.l10n_latam_document_type_id.l10n_do_ncf_type == "exterior")
            )
            line = 0
            report_data = ""
            for inv in invoice_ids:
